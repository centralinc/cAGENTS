...
fn test_import_agents_md_recursively() {
    ...
    // Verify all AGENTS.md files (root and nested) were regenerated by build
    assert!(std::path::Path::new("AGENTS.md").exists(), "Root AGENTS.md should be regenerated by build");
    assert!(std::path::Path::new("backend/AGENTS.md").exists(), "Nested backend AGENTS.md should be regenerated by build with globs");
    assert!(std::path::Path::new("frontend/AGENTS.md").exists(), "Nested frontend AGENTS.md should be regenerated by build with globs");

    // Verify nested files have correct content
    let backend_output = fs::read_to_string("backend/AGENTS.md").unwrap();
    assert!(backend_output.contains("Use REST conventions"), "Backend AGENTS.md should have original content");

    let frontend_output = fs::read_to_string("frontend/AGENTS.md").unwrap();
    assert!(frontend_output.contains("Use functional components"), "Frontend AGENTS.md should have original content");
}
